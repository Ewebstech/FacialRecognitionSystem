""" This module contains functions to manipulate images. Such as:
- cut_face
- cut_face_ellipse
- normalize_intensity
- resize
...
def resize(images, size=(100, 100)):
    ...
    for image in images:
        ...
        if image.shape < size:
            image_norm = cv2.resize(image, size, interpolation=cv2.INTER_AREA) #read up on inter area
        else:
            image_norm = cv2.resize(image, size, interpolation=cv2.INTER_CUBIC) #read up on inter cubic
...
def draw_face_eye_rectangle(image, faces_coord, eye_cascade):
    ...
    for (x, y, w, h) in faces_coord:
        ...
        # Draw rectangle around eyes inside the face formed
        eye_gray = imagegray[y:y+h, x:x+w]
        eye_color = image[y:y+h, x:x+w]
        eyes = eye_cascade.detectMultiScale(eye_gray,1.1,3)
